  ...
  eserviceRouter
    ...
    .post("/eservices/:eserviceId/scheduleArchive", async (req, res) => {
      ...
      try {
        ...
        const eservice = await eserviceService.scheduleArchiveEService(
          unsafeBrandId(req.params.eserviceId),
          req.body,
          ctx
        );
        ...
        const errorRes = makeApiProblem(
          ...
          emptyErrorMapper,
          ctx,
          `Error scheduling archiving for eservice with id ${req.params.eserviceId}`
        );
        return res.status(errorRes.status).send(errorRes);
      }
    })
      ...
      async (req, res) => {
        ...
        try {
          ...
          const errorRes = makeApiProblem(
            ...
            emptyErrorMapper,
            ctx,
            `Error approving delegated archiving request for descriptor with id ${req.params.descriptorId} for eservice with id ${req.params.eserviceId}`
          );
          return res.status(errorRes.status).send(errorRes);
        }
      }
    ...
    .post(
      ...
      async (req, res) => {
        ...
        try {
          ...
          const descriptor =
            await eserviceService.rejectDelegatedDescriptorArchiving(
              unsafeBrandId(req.params.eserviceId),
              unsafeBrandId(req.params.descriptorId),
              req.body,
              ctx
          ...
          const errorRes = makeApiProblem(
            ...
            emptyErrorMapper,
            ctx,
            `Error rejecting delegated archiving request for descriptor with id ${req.params.descriptorId} for eservice with id ${req.params.eserviceId}`
          );
          return res.status(errorRes.status).send(errorRes);
        }
      }
    ...
    .delete("/eservices/:eserviceId/scheduleArchive", async (req, res) => {
      ...
      try {
        ...
        const eservice = await eserviceService.cancelEServiceArchiving(
          unsafeBrandId(req.params.eserviceId),
          ctx
        );
        return res.status(200).send(m2mGatewayApiV3.EService.parse(eservice));
