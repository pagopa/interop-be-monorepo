...
describe("PATCH /purposes/:purposeId router test", () => {
  ...
  const mockUpdateSeed: m2mGatewayApiV3.PurposeDraftUpdateSeed = {
    title: "updated title",
  ...
  const makeRequest = async (
    ...
    body: m2mGatewayApiV3.PurposeDraftUpdateSeed = mockUpdateSeed
  ) =>
    request(api)
  ...
  it.each([
    ...
    async (seed: m2mGatewayApiV3.PurposeDraftUpdateSeed) => {
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      mockPurpose.id,
      seed as m2mGatewayApiV3.PurposeDraftUpdateSeed
    );
