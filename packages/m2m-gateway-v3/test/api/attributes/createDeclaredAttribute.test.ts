...
describe("POST /declaredAttributes router test", () => {
  ...
  const mockM2MDeclaredAttributeResponse: m2mGatewayApiV3.DeclaredAttribute =
  ...
  const makeRequest = async (
    token: string,
    body: m2mGatewayApiV3.DeclaredAttributeSeed
  ) =>
    request(api)
  ...
  it.each([
    ...
    async (body) => {
      ...
      const res = await makeRequest(
        token,
        body as m2mGatewayApiV3.DeclaredAttributeSeed
      );
