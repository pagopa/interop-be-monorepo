...
describe("POST /producerKeychains/:keychainId/eservices router test", () => {
  ...
  const makeRequest = async (
    ...
    body: m2mGatewayApiV3.ProducerKeychainAddEService
  ) =>
    request(api)
  ...
  it.each([
    {},
    { ...mockSeed, eserviceId: undefined },
    { ...mockSeed, eserviceId: "invalid-eservice-id" },
  ])("Should return 400 if passed an invalid seed", async (body) => {
    const token = generateToken(authRole.M2M_ADMIN_ROLE);
    const res = await makeRequest(
      token,
      generateId(),
      body as m2mGatewayApiV3.ProducerKeychainAddEService
    );
