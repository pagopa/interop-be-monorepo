...
describe("POST /tenants/:tenantId/certifiedDiscreteAttributes router test", () => {
  const mockApiResponse = getMockedApiCertifiedDiscreteTenantAttribute();
  const mockResponse =
    toM2MGatewayApiTenantCertifiedDiscreteAttribute(mockApiResponse);
  ...
  const makeRequest = async (
    token: string,
    body: m2mGatewayApiV3.TenantCertifiedDiscreteAttributeSeed = mockSeed,
    tenantId: TenantId = generateId()
  ) =>
    request(api)
      .post(`${appBasePath}/tenants/${tenantId}/certifiedDiscreteAttributes`)
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      mockTenantService.assignTenantCertifiedDiscreteAttribute = vi
        .fn()
  ...
  ] as m2mGatewayApiV3.TenantCertifiedDiscreteAttributeSeed[])(
