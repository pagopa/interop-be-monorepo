...
describe("POST /agreements/:agreementId/clone router test", () => {
  ...
  it.each([
    ...
    mockAgreementService.cloneAgreement = vi.fn().mockRejectedValue(error);
  ...
  it.each([
    ...
    async (resp) => {
      mockAgreementService.cloneAgreement = vi.fn().mockResolvedValueOnce(resp);
      const token = generateToken(authRole.M2M_ADMIN_ROLE);
      const res = await makeRequest(token);
