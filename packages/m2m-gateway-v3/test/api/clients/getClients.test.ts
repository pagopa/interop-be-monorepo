...
describe("GET /clients router test", () => {
  const m2mPartialClientsResponse: m2mGatewayApiV3.Clients = {
  ...
  const m2mFullClientsResponse: m2mGatewayApiV3.Clients = {
    pagination: { offset: 0, limit: 10, totalCount: 2 },
    results: [
      toM2MGatewayApiConsumerClient(
        getMockedApiConsumerFullClient({
  ...
  const mockQueryParams: m2mGatewayApiV3.GetClientsQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetClientsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as m2mGatewayApiV3.GetClientsQueryParams
    );
