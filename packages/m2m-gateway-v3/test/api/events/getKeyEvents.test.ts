...
describe("GET /keyEvents router test", () => {
  const mockKeyEvents: m2mGatewayApiV3.KeyEvents = {
    events: [
      {
        id: generateId(),
  ...
  const mockQueryParams: m2mGatewayApiV3.GetEventManagerKeysQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerKeysQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerKeysQueryParams
    );
