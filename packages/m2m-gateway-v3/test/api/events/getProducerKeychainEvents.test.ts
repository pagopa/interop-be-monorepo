...
describe("GET /producerKeychainEvents router test", () => {
  const mockProducerKeychainEvents: m2mGatewayApiV3.ProducerKeychainEvents = {
    events: [
      {
        id: generateId(),
  ...
  const mockQueryParams: m2mGatewayApiV3.GetEventManagerProducerKeychainEventsQueryParams =
    {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerProducerKeychainEventsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerProducerKeychainEventsQueryParams
    );
