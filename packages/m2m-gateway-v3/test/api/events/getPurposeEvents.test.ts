...
describe("GET /purposeEvents router test", () => {
  ...
  const mockPurposeEvents: m2mGatewayApiV3.PurposeEvents = {
    events,
  };

  const mockQueryParams: m2mGatewayApiV3.GetEventManagerPurposesQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerPurposesQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerPurposesQueryParams
    );
