...
describe("GET /clientEvents router test", () => {
  const mockClientEvents: m2mGatewayApiV3.ClientEvents = {
    events: [
      {
        id: generateId(),
  ...
  const mockQueryParams: m2mGatewayApiV3.GetEventManagerClientQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerClientQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerClientQueryParams
    );
