...
describe("GET /tenantEvents router test", () => {
  ...
  const mockTenantEvents: m2mGatewayApiV3.TenantEvents = {
    events,
  };
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerTenantEventsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerTenantEventsQueryParams
    );
