...
describe("GET /eserviceTemplateEvents router test", () => {
  const events: m2mGatewayApiV3.EServiceTemplateEvent[] = [
    {
      id: generateId(),
  ...
  const mockEServiceTemplateEvents: m2mGatewayApiV3.EServiceTemplateEvents = {
    events,
  };
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerEServiceTemplatesQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerEServiceTemplatesQueryParams
    );
