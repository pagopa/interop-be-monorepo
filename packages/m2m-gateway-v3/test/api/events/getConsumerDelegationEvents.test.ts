...
describe("GET /consumerDelegationEvents router test", () => {
  const events: m2mGatewayApiV3.ConsumerDelegationEvent[] = [
    {
      id: generateId(),
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerConsumerDelegationsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerConsumerDelegationsQueryParams
    );
