...
describe("GET /agreementEvents router test", () => {
  const events: m2mGatewayApiV3.AgreementEvent[] = [
    {
      id: generateId(),
  ...
  const mockAgreementEvents: m2mGatewayApiV3.AgreementEvents = {
    events,
  };
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerAgreementsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerAgreementsQueryParams
    );
