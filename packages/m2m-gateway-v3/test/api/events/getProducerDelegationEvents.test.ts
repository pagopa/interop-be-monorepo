...
describe("GET /producerDelegationEvents router test", () => {
  const events: m2mGatewayApiV3.ProducerDelegationEvent[] = [
    {
      id: generateId(),
  ...
  const mockProducerDelegationEvents: m2mGatewayApiV3.ProducerDelegationEvents =
    {
      events,
    };

  const mockQueryParams: m2mGatewayApiV3.GetEventManagerProducerDelegationsQueryParams =
    {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerProducerDelegationsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerProducerDelegationsQueryParams
    );
