...
describe("GET /eserviceEvents router test", () => {
  const events: m2mGatewayApiV3.EServiceEvent[] = [
    {
      id: generateId(),
  ...
  const mockEServiceEvents: m2mGatewayApiV3.EServiceEvents = {
    events,
  };

  const mockQueryParams: m2mGatewayApiV3.GetEventManagerEServicesQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEventManagerEServicesQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEventManagerEServicesQueryParams
    );
