...
describe("POST /eservices/:eserviceId/submitDelegatedArchiving router test", () => {
  const mockEService: catalogApi.EService = getMockedApiEservice();

  const mockSeed: m2mGatewayApiV3.DelegateEServiceArchivingSeed = {
  ...
  const makeRequest = async (
    ...
    body: m2mGatewayApiV3.DelegateEServiceArchivingSeed = mockSeed
  ) =>
    request(api)
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      mockEserviceService.submitDelegatedEServiceArchiving = vi
        .fn()
      ...
      expect(
        mockEserviceService.submitDelegatedEServiceArchiving
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      mockEService.id,
      seed as m2mGatewayApiV3.DelegateEServiceArchivingSeed
    );
