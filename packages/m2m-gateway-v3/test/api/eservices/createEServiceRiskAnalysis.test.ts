...
describe("POST /eservice/:eserviceId/riskAnalyses router test", () => {
  ...
  const mockM2MEserviceResponse: m2mGatewayApiV3.EServiceRiskAnalysis =
    toM2MGatewayApiEServiceRiskAnalysis(mockRiskAnalysis);

  const makeRequest = async (
    token: string,
    eserviceId: string,
    body: m2mGatewayApiV3.EServiceRiskAnalysisSeed
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      mockEService.id,
      body as unknown as m2mGatewayApiV3.EServiceRiskAnalysisSeed
    );
