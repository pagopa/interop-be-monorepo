...
describe("POST /eserviceTemplates router test", () => {
  ...
  const mockM2MEserviceTemplateResponse: m2mGatewayApiV3.EServiceTemplate =
    toM2MGatewayEServiceTemplate(mockApiEserviceTemplate);

  const makeRequest = async (
    token: string,
    body: m2mGatewayApiV3.EServiceTemplateSeed
  ) =>
    request(api)
  ...
  it.each([
    ...
    async (body) => {
      ...
      const res = await makeRequest(
        token,
        body as m2mGatewayApiV3.EServiceTemplateSeed
      );
