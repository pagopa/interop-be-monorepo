...
describe("GET /eserviceTemplates router test", () => {
  ...
  const mockM2MEServiceTemplatesResponse: m2mGatewayApiV3.EServiceTemplates = {
  ...
  const mockQueryParams: m2mGatewayApiV3.GetEServiceTemplatesQueryParams = {
    offset: 0,
    limit: 10,
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetEServiceTemplatesQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mGatewayApiV3.GetEServiceTemplatesQueryParams
    );
