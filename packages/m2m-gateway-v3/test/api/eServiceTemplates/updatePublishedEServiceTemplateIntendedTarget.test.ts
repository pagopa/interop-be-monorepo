...
describe("PATCH /eserviceTemplates/:templateId/intendedTarget router test", () => {
  const mockEServiceTemplate: eserviceTemplateApi.EServiceTemplate =
    getMockedApiEServiceTemplate();

  const mockSeed: m2mGatewayApiV3.EServiceTemplateIntendedTargetUpdateSeed = {
  ...
  const makeRequest = async (
    ...
    body: m2mGatewayApiV3.EServiceTemplateIntendedTargetUpdateSeed = mockSeed
  ) =>
    request(api)
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      mockEServiceTemplateService.updatePublishedEServiceTemplateIntendedTarget =
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      mockEServiceTemplate.id,
      seed as m2mGatewayApiV3.EServiceTemplateIntendedTargetUpdateSeed
    );
