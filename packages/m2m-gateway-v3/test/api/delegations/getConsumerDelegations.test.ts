...
describe("GET /consumerDelegations router test", () => {
  ...
  const mockM2MDelegationsResponse: m2mGatewayApiV3.ConsumerDelegations = {
  ...
  const mockQueryParams: m2mGatewayApiV3.GetConsumerDelegationsQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApiV3.GetConsumerDelegationsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as m2mGatewayApiV3.GetConsumerDelegationsQueryParams
    );
