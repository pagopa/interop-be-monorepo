...
describe("createEServiceDescriptorCertifiedDiscreteAttributesGroup", () => {
  ...
  const response: m2mGatewayApiV3.EServiceDescriptorCertifiedDiscreteAttributesGroup =
    {
      attributes: [
        {
          groupIndex: 1,
  ...
  it("Should throw missingMetadata in case the attribute returned by the update PATCH call has no metadata", async () => {
    ...
    await expect(
      eserviceService.createEServiceDescriptorCertifiedDiscreteAttributesGroup(
        unsafeBrandId(eservice.id),
        unsafeBrandId(descriptor.id),
        seed,
        getMockM2MAdminAppContext()
