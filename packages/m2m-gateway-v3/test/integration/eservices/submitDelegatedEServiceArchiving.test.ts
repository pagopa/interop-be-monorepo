...
describe("submitDelegatedEServiceArchiving", () => {
  ...
  const mockSubmitDelegatedEServiceArchiving = vi
    .fn()
  ...
  mockInteropBeClients.catalogProcessClient = {
    ...
    submitDelegatedEServiceArchiving: mockSubmitDelegatedEServiceArchiving,
  } as unknown as PagoPAInteropBeClients["catalogProcessClient"];

  beforeEach(() => {
    mockSubmitDelegatedEServiceArchiving.mockClear();
    mockGetEService.mockClear();
  });

  it("Should succeed and perform service calls", async () => {
    const result = await eserviceService.submitDelegatedEServiceArchiving(
  ...
  it.each(
    ...
    const result = await eserviceService.submitDelegatedEServiceArchiving(
      unsafeBrandId(mockEService.id),
      body as m2mGatewayApiV3.DelegateEServiceArchivingSeed,
      getMockM2MAdminAppContext()
    );
  ...
  it("Should throw missingMetadata in case the eservice returned by the POST call has no metadata", async () => {
    mockSubmitDelegatedEServiceArchiving.mockResolvedValueOnce({
      ...mockEServiceProcessGetResponse,
      metadata: undefined,
    });

    await expect(
      eserviceService.submitDelegatedEServiceArchiving(
        unsafeBrandId(mockEService.id),
        mockSeed,
        getMockM2MAdminAppContext()
      )
