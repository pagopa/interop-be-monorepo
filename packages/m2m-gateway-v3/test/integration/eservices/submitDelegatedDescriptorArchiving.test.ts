...
describe("submitDelegatedDescriptorArchiving", () => {
  ...
  const mockSubmitDelegatedDescriptorArchiving = vi
    .fn()
  ...
  mockInteropBeClients.catalogProcessClient = {
    ...
    submitDelegatedDescriptorArchiving: mockSubmitDelegatedDescriptorArchiving,
  } as unknown as PagoPAInteropBeClients["catalogProcessClient"];

  beforeEach(() => {
    mockSubmitDelegatedDescriptorArchiving.mockClear();
    mockGetEService.mockClear();
  });

  it("Should succeed and perform service calls", async () => {
    const result = await eserviceService.submitDelegatedDescriptorArchiving(
    ...
    expectApiClientPostToHaveBeenCalledWith({
      mockPost:
        mockInteropBeClients.catalogProcessClient
          .submitDelegatedDescriptorArchiving,
      params: {
  ...
  it.each([30, 60, 90, 120])(
    ...
    async (gracePeriodDays) => {
      ...
      const result = await eserviceService.submitDelegatedDescriptorArchiving(
        unsafeBrandId(mockApiEservice.id),
        unsafeBrandId(mockApiDescriptor.id),
        seed as m2mGatewayApiV3.DelegateGracePeriodDaysSeed,
        getMockM2MAdminAppContext()
      );
      const expectedM2MEServiceDescriptor: m2mGatewayApiV3.EServiceDescriptor =
        toM2MGatewayApiEServiceDescriptor(mockApiDescriptor);
  ...
  it("Should throw missingMetadata in case the eservice returned by the POST call has no metadata", async () => {
    mockSubmitDelegatedDescriptorArchiving.mockResolvedValueOnce({
      ...mockEServiceProcessPostResponse,
      metadata: undefined,
    });

    await expect(
      eserviceService.submitDelegatedDescriptorArchiving(
        unsafeBrandId(mockApiEservice.id),
        unsafeBrandId(mockApiDescriptor.id),
        mockSeed,
        getMockM2MAdminAppContext()
