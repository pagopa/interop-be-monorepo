...
describe("getCertifiedDiscreteAttribute", () => {
  ...
  it("Should succeed and perform API clients calls", async () => {
    ...
    const result = await attributeService.getCertifiedDiscreteAttribute(
  ...
  it("Should throw attributeNotFound in case the returned attribute has an unexpected kind", async () => {
    ...
    await expect(
      attributeService.getCertifiedDiscreteAttribute(
        mockAttributeProcessResponse.data.id,
        getMockM2MAdminAppContext()
      )
    ).rejects.toThrow(attributeNotFound(mockResponse.data));
  });
