...
describe("createEServiceTemplateVersionCertifiedDiscreteAttributesGroup", () => {
  ...
  const response: m2mGatewayApiV3.EServiceTemplateVersionCertifiedDiscreteAttributesGroup =
    {
      attributes: [
        {
          groupIndex: 1,
  ...
  it("Should throw missingMetadata in case the attribute returned by the update PATCH call has no metadata", async () => {
    ...
    await expect(
      eserviceTemplateService.createEServiceTemplateVersionCertifiedDiscreteAttributesGroup(
        unsafeBrandId(eserviceTemplate.id),
        unsafeBrandId(version.id),
        seed,
        getMockM2MAdminAppContext()
