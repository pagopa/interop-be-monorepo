...
describe("createEServiceTemplateVersionCertifiedAttributesGroup", () => {
  ...
  const response: m2mGatewayApiV3.EServiceTemplateVersionCertifiedAttributesGroup =
  {
    attributes: [
      {
        groupIndex: 1,
  ...
  it("Should throw missingMetadata in case the attribute returned by the update PATCH call has no metadata", async () => {
    ...
    await expect(
      eserviceTemplateService.createEServiceTemplateVersionCertifiedAttributesGroup(
        unsafeBrandId(eserviceTemplate.id),
        unsafeBrandId(version.id),
        seed,
        getMockM2MAdminAppContext()
