...
import { clientKeyToJWKKey } from "pagopa-interop-commons";
...
describe("Events V2", () => {
  ...
  it("ClientKeyAdded", async () => {
    ...
    const jwkKey = clientKeyToJWKKey(mockKey);
    ...
    expect(retrievedKey?.data).toEqual(clientKeyToJWKKey(addedKey));
  ...
  it("ClientDeleted", async () => {
    ...
    const jwkKey1 = clientKeyToJWKKey(mockKey1);
    const jwkKey2 = clientKeyToJWKKey(mockKey2);
    const mockClient: Client = {
      ...getMockClient(),
      id: clientId,
