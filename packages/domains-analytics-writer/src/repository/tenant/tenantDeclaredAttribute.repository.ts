...
import {
  TenantDeclaredAttributeMapping,
  TenantDeclaredAttributeSchema,
} from "../../model/tenant/tenantDeclaredAttribute.js";
...
export function tenantDeclaredAttributeRepository(conn: DBConnection) {
  ...
  return {
    async insert(
      t: ITask<unknown>,
      pgp: IMain,
      records: TenantDeclaredAttributeSQL[]
    ): Promise<void> {
      try {
        const mapping: TenantDeclaredAttributeMapping = {
        ...
        const cs = buildColumnSet<TenantDeclaredAttributeSQL>(
          pgp,
          mapping,
          stagingTable
        );
