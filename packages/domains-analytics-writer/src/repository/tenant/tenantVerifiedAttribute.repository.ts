...
import {
  TenantVerifiedAttributeMapping,
  TenantVerifiedAttributeSchema,
} from "../../model/tenant/tenantVerifiedAttribute.js";
...
export function tenantVerifiedAttributeRepository(conn: DBConnection) {
  ...
  return {
    async insert(
      t: ITask<unknown>,
      pgp: IMain,
      records: TenantVerifiedAttributeSQL[]
    ): Promise<void> {
      try {
        const mapping: TenantVerifiedAttributeMapping = {
        ...
        const cs = buildColumnSet<TenantVerifiedAttributeSQL>(
          pgp,
          mapping,
          stagingTable
        );
