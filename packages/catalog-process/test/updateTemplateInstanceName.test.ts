...
describe("updateTemplateInstanceName", () => {
  it("should write on event-store for the internal update of the eService name without instanceName", async () => {
  ...
  it("should write on event-store for the internal update of the eService name with instanceName", async () => {
    const descriptor: Descriptor = {
      ...getMockDescriptor(descriptorState.published),
      interface: getMockDocument(),
    };
    ...
    const eservice: EService = {
      ...getMockEService(),
      descriptors: [descriptor],
      templateRef: { id: generateId(), instanceName: "test" },
    };
