...
describe("publish descriptor", () => {
  ...
  it("should publish a new Receive descriptor even if an already published eService has a risk analysis with a previously allowed tenant kind", async () => {
    ...
    const expectedPublishedDescriptor: Descriptor = {
      ...publishedDescriptor,
      archivedAt: new Date(),
      state: descriptorState.archived,
    };
    const expectedDraftDescriptor: Descriptor = {
      ...draftDescriptor,
      publishedAt: new Date(),
      state: descriptorState.published,
    };
    const expectedEservice: EService = {
      ...eservice,
      descriptors: [expectedPublishedDescriptor, expectedDraftDescriptor],
    };
  ...
  it("should throw missingAsyncExchangeProperties if asyncExchange is true and descriptor asyncExchangeProperties is undefined", async () => {
    const descriptor: Descriptor = {
      ...mockDescriptor,
      state: descriptorState.draft,
      interface: mockDocument,
