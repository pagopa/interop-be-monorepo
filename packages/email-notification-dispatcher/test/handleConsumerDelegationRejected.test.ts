...
import { handleConsumerDelegationRejected } from "../src/handlers/delegations/handleConsumerDelegationRejected.js";
import {
  addOneDelegation,
  addOneEService,
  addOneTenant,
...
describe("handleConsumerDelegationRejected", async () => {
  ...
  it("should throw missingKafkaMessageDataError when delegation is undefined", async () => {
    await expect(() =>
      ...
      missingKafkaMessageDataError("delegation", "ConsumerDelegationRejected")
    );
  });
  ...
  it("should generate one message per user of the delegator", async () => {
    ...
    const messages = await handleConsumerDelegationRejected({
      delegationV2Msg: toDelegationV2(delegation),
      logger,
      templateService,
      userService,
