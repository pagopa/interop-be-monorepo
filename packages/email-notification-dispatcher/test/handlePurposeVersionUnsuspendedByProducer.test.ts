...
describe("handlePurposeVersionUnsuspendedByProducer", async () => {
  ...
  it("should generate one message per user of the tenant that consumed the eservice", async () => {
    ...
    const messages = await handlePurposeVersionUnsuspendedByProducer({
      purposeV2Msg: toPurposeV2(purpose),
      logger,
      templateService,
      userService,
  ...
  it("should generate a complete and correct message", async () => {
    ...
    messages.forEach((message) => {
      ...
      expect(message.email.body).toContain(
        `Riattivazione della finalità &quot;${purpose.title}&quot;`
      );
