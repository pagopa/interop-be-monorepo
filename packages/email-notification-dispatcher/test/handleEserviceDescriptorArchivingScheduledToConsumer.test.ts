...
import { handleEserviceDescriptorArchivingScheduledToConsumer } from "../src/handlers/eservices/handleEserviceDescriptorArchivingScheduledToConsumer.js";
import {
  addOneAgreement,
  addOneEService,
  addOneTenant,
...
describe("handleEserviceDescriptorArchivingScheduledToConsumer", () => {
  ...
  it("emits one email per consumer user", async () => {
    ...
    expect(messages[0].email.subject).toContain(
      'Avviso di archiviazione per l\'e-service "Test E-service"'
    );
  });
