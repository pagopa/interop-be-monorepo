...
describe("handleAgreementSubmitted", async () => {
  ...
  it("should throw missingKafkaMessageDataError when agreement is undefined", async () => {
    await expect(() =>
      ...
      missingKafkaMessageDataError("agreement", "AgreementSubmitted")
    );
  });
  ...
  it("should generate one message to the producer of the eservice for the agreement", async () => {
    const agreement: Agreement = {
      ...getMockAgreement(),
      stamps: { activation: { when: new Date(), who: generateId<UserId>() } },
      producerId: producerTenant.id,
