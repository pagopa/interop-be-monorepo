...
import { handleAgreementSuspendedByConsumer } from "../src/handlers/agreements/handleAgreementSuspendedByConsumer.js";
import {
  addOneAgreement,
  addOneEService,
  addOneTenant,
...
describe("handleAgreementSuspendedByConsumer", async () => {
  ...
  it("should throw missingKafkaMessageDataError when agreement is undefined", async () => {
    await expect(() =>
      ...
      missingKafkaMessageDataError("eservice", "AgreementSuspendedByConsumer")
    );
  });
  ...
  it("should generate one message per user of the tenant that produced the eservice", async () => {
    ...
    const messages = await handleAgreementSuspendedByConsumer({
      agreementV2Msg: toAgreementV2(agreement),
      logger,
      templateService,
      userService,
  ...
  it("should generate a complete and correct message", async () => {
    ...
    messages.forEach((message) => {
      ...
      expect(message.email.body).toContain(
        `Sospensione richiesta di fruizione da parte del fruitore`
      );
