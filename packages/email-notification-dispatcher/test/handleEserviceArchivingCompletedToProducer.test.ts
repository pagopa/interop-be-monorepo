...
import { handleEserviceArchivingCompletedToProducer } from "../src/handlers/eservices/handleEserviceArchivingCompletedToProducer.js";
import {
  addOneEService,
  addOneTenant,
  getMockUser,
...
describe("handleEserviceArchivingCompletedToProducer", () => {
  ...
  it.each([...gracePeriodDays])(
    ...
    async (gracePeriodDaysValue: GracePeriodDays) => {
      ...
      const messages = await handleEserviceArchivingCompletedToProducer({
      ...
      expect(messages[0].email.subject).toContain(
        "Un tuo e-service è stato archiviato"
      );
    }
  );
});
