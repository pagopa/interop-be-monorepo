...
import { handleConsumerDelegationSubmitted } from "../src/handlers/delegations/handleConsumerDelegationSubmitted.js";
import {
  addOneDelegation,
  addOneEService,
  addOneTenant,
...
describe("handleConsumerDelegationSubmitted", async () => {
  ...
  it("should throw missingKafkaMessageDataError when delegation is undefined", async () => {
    await expect(() =>
      handleConsumerDelegationSubmitted({
      ...
      missingKafkaMessageDataError("delegation", "ConsumerDelegationSubmitted")
    );
  });
  ...
  it("should generate one message per user of the delegate", async () => {
    ...
    const messages = await handleConsumerDelegationSubmitted({
      delegationV2Msg: toDelegationV2(delegation),
      logger,
      templateService,
      userService,
  ...
  it("should generate a complete and correct message", async () => {
    ...
    messages.forEach((message) => {
      ...
      expect(message.email.body).toContain(
        `Hai ricevuto una richiesta di delega`
      );
      match(message.type)
        .with("User", () => {
