...
import { handleEserviceArchivingRequestedByDelegate } from "../src/handlers/eservices/handleEserviceArchivingRequestedByDelegate.js";
import {
  addOneDelegation,
  addOneEService,
  addOneTenant,
...
describe("handleEserviceArchivingRequestedByDelegate", async () => {
  ...
  it("should generate one message per delegator user with the expected subject", async () => {
    const messages = await handleEserviceArchivingRequestedByDelegate({
    ...
    expect(
      messages.every(
        (m) =>
          m.email.subject === `Nuova richiesta di archiviazione di un e-service`
      )
    ).toBe(true);
    expect(
