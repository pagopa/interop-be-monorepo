...
import { handleProducerDelegationSubmitted } from "../src/handlers/delegations/handleProducerDelegationSubmitted.js";
import {
  addOneDelegation,
  addOneEService,
  addOneTenant,
...
describe("handleProducerDelegationSubmitted", async () => {
  ...
  it("should throw missingKafkaMessageDataError when delegation is undefined", async () => {
    await expect(() =>
      handleProducerDelegationSubmitted({
  ...
  it("should generate one message per user of the delegate", async () => {
    ...
    const messages = await handleProducerDelegationSubmitted({
      delegationV2Msg: toDelegationV2(delegation),
      logger,
      templateService,
      userService,
