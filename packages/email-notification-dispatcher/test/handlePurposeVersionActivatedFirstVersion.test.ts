...
import { handlePurposeVersionActivatedFirstVersion } from "../src/handlers/purposes/handlePurposeVersionActivated.js";
import {
  addOneEService,
  addOnePurpose,
  addOneTenant,
...
describe("handlePurposeVersionActivated", async () => {
  ...
  it("should throw missingKafkaMessageDataError when purpose is undefined", async () => {
    await expect(() =>
      handlePurposeVersionActivatedFirstVersion({
  ...
  it("should generate one message per user of the consumer", async () => {
    ...
    const messages = await handlePurposeVersionActivatedFirstVersion({
      purposeV2Msg: toPurposeV2(purpose),
      logger,
      templateService,
      readModelService,
