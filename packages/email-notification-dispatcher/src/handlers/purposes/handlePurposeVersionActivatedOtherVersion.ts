...
export async function handlePurposeVersionActivatedOtherVersion(
  ...
  if (purpose.versions.length <= 1) {
    logger.info(
      `Skipping email notification for handlePurposeVersionActivatedOtherVersion - entityId: ${purpose.id}, eventType: ${notificationType}, reason: purpose has only one version`
    );
    return [];
  }
  ...
  if (targets.length === 0) {
    logger.info(
      `No users with email notifications enabled for handlePurposeVersionActivatedOtherVersion - entityId: ${purpose.id}, eventType: ${notificationType}`
    );
    return [];
  }
  ...
  return targets.map((t) => ({
    ...
    email: {
      subject: `Richiesta di adeguamento piano di carico accettata per la finalità "${purpose.title}"`,
      body: templateService.compileHtml(htmlTemplate, {
        title: `Richiesta di adeguamento piano di carico accettata per la finalità "${purpose.title}"`,
        notificationType,
        entityId: purpose.id,
