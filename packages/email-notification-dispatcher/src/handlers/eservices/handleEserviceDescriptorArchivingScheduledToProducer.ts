...
export async function handleEserviceDescriptorArchivingScheduledToProducer(
  ...
  const [htmlTemplate, producer] = await Promise.all([
    retrieveHTMLTemplate(
      eventMailTemplateType.eserviceArchivingStartedDescriptorToProducerMailTemplate
    ),
  ...
  if (targets.length === 0) {
    logger.info(
      `No producer users with email notifications enabled for handleEserviceDescriptorArchivingScheduledToProducer - entityId: ${eservice.id}/${descriptor.id}`
    );
    return [];
  }
  ...
  const archivableOn = descriptor.archivingSchedule
    ...
    : undefined;
  const subject = `Una versione di un tuo e-service è in fase di archiviazione`;
