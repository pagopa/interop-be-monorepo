...
export async function handleAgreementSuspendedByConsumer(
  ...
  if (targets.length === 0) {
    logger.info(
      `No users with email notifications enabled for handleAgreementSuspendedByConsumer - entityId: ${agreement.id}, eventType: ${notificationType}`
    );
    return [];
  }
  ...
  return targets.map((t) => ({
    ...
    email: {
      subject: `Sospensione richiesta di fruizione da parte del fruitore`,
      body: templateService.compileHtml(htmlTemplate, {
        title: `Sospensione richiesta di fruizione da parte del fruitore`,
        notificationType,
