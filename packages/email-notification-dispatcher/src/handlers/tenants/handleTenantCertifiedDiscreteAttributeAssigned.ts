...
export async function handleTenantCertifiedDiscreteAttributeAssigned(
  ...
  if (targets.length === 0) {
    logger.info(
      `No users with email notifications enabled for handleTenantCertifiedDiscreteAttributeAssigned - entityId: ${tenant.id}, eventType: ${notificationType}`
    );
    return [];
  }
