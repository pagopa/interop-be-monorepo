...
| `jwkDecodingError` | 400 | The submitted PEM cannot be decoded into a usable JWK structure (`createJWK`). | **CAN HAPPEN** — same as `notAllowedPrivateKeyException`; the FE has no client-side decode or structure validation before the request is sent. | Same as `notAllowedPrivateKeyException`, except the pasted value is malformed or not decodable as JWK material. | 🟢 Easy resolution <br />1. Copy the public key again from the original source.<br>2. Paste the complete key without line breaks or truncation and retry. |
