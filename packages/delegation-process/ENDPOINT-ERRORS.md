...
| Error                            | Code       | Status | When it happens                                                                                                                                                               | Reachable from the FE?                                                                                                                                               | Steps to reproduce (UI)                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------------------- | ---------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `eserviceNotFound`               | `010-0002` | 400    | `retrieveEserviceById` cannot find the selected e-service.                                                                                                                    | Cannot happen — the FE sends an e-service selected from the producer e-service autocomplete; stale selection is out of scope.                                        | —                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `tenantNotFound`                 | `010-0004` | 400    | `retrieveTenantById` cannot find either the requester or selected delegate tenant.                                                                                            | Cannot happen — both ids come from authenticated tenant data or the tenant autocomplete; crafted or stale ids are out of scope.                                      | —                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `invalidDelegatorAndDelegateIds` | `010-0005` | 400    | `assertDelegatorIsNotDelegate` receives the same tenant id for delegator and delegate.                                                                                        | Cannot happen — `DelegationCreateTenantAutocomplete` supplies a delegate option distinct from the current organization.                                              | —                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `eserviceAlreadyArchived`        | `010-0016` | 400    | `assertEserviceIsNotArchived` finds the latest descriptor in `ARCHIVED` state.                                                                                                | Cannot happen — the FE producer e-service autocomplete exposes selectable active e-services, not archived ones.                                                      | —                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `delegationNotAllowedForTenant`  | `010-0006` | 403    | `assertDelegatorAndDelegateAllowedForDelegation` finds no active delegation-allowed certified attribute on the delegator or delegate, when the feature-flag skip is disabled. | Cannot happen — the delegate autocomplete is filtered to tenants available for delegation and the current tenant’s availability guard controls the delegation route. | —                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `tenantNotAllowedToDelegation`   | `010-0007` | 403    | `assertTenantAllowedToReceiveDelegation` finds no `DELEGATED_PRODUCER` feature on the selected delegate.                                                                      | Cannot happen — the provider delegate autocomplete states and filters to tenants that have given availability to become producer delegates.                          | —                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `differentEserviceProducer`      | `010-0012` | 403    | `assertDelegatorIsProducer` sees that the selected e-service producer is not the authenticated delegator.                                                                     | Cannot happen — the producer e-service autocomplete is scoped to e-services owned by the current organization.                                                       | —                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `delegationAlreadyExists`        | `010-0003` | 409    | `assertDelegationNotExists` finds an active or waiting producer delegation for the same delegator, e-service, and kind.                                                       | **CAN HAPPEN** — the producer delegation form does not query or disable on an existing producer delegation before submitting.                                        | **Data precondition:** tenant A owns active e-service E and already has a producer delegation for E to tenant B.<br>1. Open **Gestione delle deleghe** and choose **Crea delega**.<br>2. Select **Delega all’erogazione**.<br>3. Select E in **Nome dell'e-service (richiesto)** and B in **Ente da delegare all'erogazione (richiesto)**.<br>4. Click **Inoltra delega**, confirm **Ho capito**, then click **Inoltra delega**. |
