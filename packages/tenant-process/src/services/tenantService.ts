...
const retrieveTenantByExternalId = async ({
  tenantOrigin,
  tenantExternalId,
  tenantSelfcareInstitutionType,
  readModelService,
}: {
  tenantOrigin: string;
  tenantExternalId: string;
  tenantSelfcareInstitutionType: string;
  readModelService: ReadModelServiceSQL;
}): Promise<WithMetadata<Tenant>> => {
  const tenant = await readModelService.getTenantByExternalId({
    origin: tenantOrigin,
    value: tenantExternalId,
    selfcareInstitutionType: tenantSelfcareInstitutionType,
  });
  if (!tenant) {
  ...
  return {
    ...
    async getTenantByExternalId(
      ...
      const tenant = await retrieveTenantByExternalId({
        ...
        tenantSelfcareInstitutionType: externalId.selfcareInstitutionType,
        readModelService,
      });
    ...
    async internalUpsertTenant(
      ...
      const existingTenant = await retrieveTenantByExternalId({
        ...
        tenantSelfcareInstitutionType:
          internalTenantSeed.selfcareInstitutionType.value,
        readModelService,
      });
    ...
    async m2mUpsertTenant(
      ...
      const existingTenant = await retrieveTenantByExternalId({
        ...
        tenantSelfcareInstitutionType:
          m2mTenantSeed.selfcareInstitutionType.value,
        readModelService,
      });
