import { beforeEach, describe, expect, it } from "vitest";
import { getMockContextM2M } from "pagopa-interop-commons-test";
import { getMockedAttributeM2MEvent } from "../mockUtils.js";
import {
  m2mEventService,
  writeAttributeM2MEvent,
} from "../integrationUtils.js";

describe("getAttributeM2MEvents", () => {
  const mockAttributeM2MEvents = [
    getMockedAttributeM2MEvent("AttributeAdded"),
    getMockedAttributeM2MEvent("AttributeAdded"),
    getMockedAttributeM2MEvent("AttributeAdded"),
    getMockedAttributeM2MEvent("MaintenanceAttributeDeleted"),
  ];

  beforeEach(async () => {
    await Promise.all(mockAttributeM2MEvents.map(writeAttributeM2MEvent));
  });

  it.each([1, 3, 10])(
    "should list the %d newest attribute M2M events if lastEventId is not provided",
    async (limit) => {
      const events = await m2mEventService.getAttributeM2MEvents(
        undefined,
        limit,
        getMockContextM2M({})
      );
      expect(events).toEqual(
        mockAttributeM2MEvents
          .slice(-limit) // get the last N events
          .reverse() // newest first
      );
    }
  );

  it.each([1, 3, 10])(
    "should list the %d newest attribute M2M events after the given lastEventId",
    async (limit) => {
      const lastEventId = mockAttributeM2MEvents[1].id;
      const events = await m2mEventService.getAttributeM2MEvents(
        lastEventId,
        limit,
        getMockContextM2M({})
      );

      const filteredEvents = mockAttributeM2MEvents.filter(
        (e) => e.id > lastEventId
      );
      expect(events).toEqual(
        filteredEvents
          .slice(-limit) // get the last N events
          .reverse() // newest first
      );
    }
  );
});
