...
import { generateM2MEventId, getMockedClientM2MEvent } from "../mockUtils.js";
import { api, m2mEventService } from "../vitest.api.setup.js";
import { testToUpperSnakeCase } from "../utils.js";

describe("API /events/clients test", () => {
  ...
  const mockClientM2MEventsResponse: m2mEventApi.ClientM2MEvents = {
    events: mockClientM2MEvents.map(
      (e) =>
        ({
          id: e.id,
  ...
  const mockQueryParams: m2mEventApi.GetClientM2MEventsQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mEventApi.GetClientM2MEventsQueryParams = mockQueryParams
  ) =>
    request(api)
      .get(`/events/clients`)
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      m2mEventService.getClientM2MEvents = vi
        .fn()
        .mockResolvedValue(mockClientM2MEvents);

      const token = generateToken(role);
      const res = await makeRequest(token);

      expect(res.status).toBe(200);
      expect(res.body).toEqual(mockClientM2MEventsResponse);
      expect(m2mEventService.getClientM2MEvents).toHaveBeenCalledWith(
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as unknown as m2mEventApi.GetClientM2MEventsQueryParams
    );
