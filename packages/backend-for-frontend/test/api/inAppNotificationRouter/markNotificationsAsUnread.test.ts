...
describe("API POST /inAppNotifications/bulk/markAsUnread", () => {
  beforeEach(() => {
    services.inAppNotificationService.markNotificationsAsUnread = vi
      .fn()
  ...
  const makeRequest = async (token: string, body: { ids: string[] }) =>
    request(api)
      .post(`${appBasePath}/inAppNotifications/bulk/markAsUnread`)
