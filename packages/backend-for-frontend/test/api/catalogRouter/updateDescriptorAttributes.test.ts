...
describe("API POST /eservices/:eServiceId/descriptors/:descriptorId/attributes/update", () => {
  ...
  const mockDescriptorAttributesSeed: bffApi.DescriptorAttributesSeed = {
    certified: [],
    declared: [],
    verified: [],
  };
  ...
  const makeRequest = async (
    ...
    request(api)
      .post(
        `${appBasePath}/eservices/${mockEServiceId}/descriptors/${descriptorId}/attributes/update`
      )
      .set("Authorization", `Bearer ${token}`)
      .set("X-Correlation-Id", generateId())
      .send(mockDescriptorAttributesSeed);
