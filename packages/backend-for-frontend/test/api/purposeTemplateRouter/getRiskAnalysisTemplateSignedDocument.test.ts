...
describe("API GET /purposeTemplates/{purposeTemplateId}/signedDocument test", () => {
  ...
  const mockSignedDocumentResponse = Buffer.from(mockSignedDocument);

  beforeEach(() => {
    services.purposeTemplateService.getRiskAnalysisTemplateSignedDocument = vi
      .fn()
      .mockResolvedValue(mockSignedDocument);
  });
  ...
  it("Should return 200 for user with role Admin", async () => {
    ...
    expect(res.body).toEqual(mockSignedDocumentResponse);
  });
