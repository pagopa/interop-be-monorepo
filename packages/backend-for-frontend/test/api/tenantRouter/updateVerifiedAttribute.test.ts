...
describe("API POST /tenants/{tenantId}/attributes/verified/{attributeId} test", () => {
  const updateSeed: bffApi.UpdateVerifiedTenantAttributeSeed = {
    expirationDate: new Date().toISOString(),
  };

  beforeEach(() => {
    clients.tenantProcessClient.tenant.updateVerifiedAttribute = vi
      .fn()
  ...
  const makeRequest = async (
    ...
    body: bffApi.UpdateVerifiedTenantAttributeSeed = updateSeed
  ) =>
    request(api)
      .post(
  ...
  it.each([
    ...
    async ({ tenantId, attributeId, body }) => {
      ...
      const res = await makeRequest(
        ...
        tenantId,
        attributeId,
        body as bffApi.UpdateVerifiedTenantAttributeSeed
      );
