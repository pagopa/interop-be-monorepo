...
describe("getProducerKeychainKeys", () => {
  const mockParams: m2mGatewayApi.GetProducerKeychainKeysQueryParams = {
    offset: 0,
    limit: 10,
  };
  ...
  const expectedM2MJWK1: m2mGatewayApi.JWK = {
    kid: mockApiProducerKeychainJWK1.kid,
  ...
  const expectedM2MJWK2: m2mGatewayApi.JWK = {
    kid: mockApiProducerKeychainJWK2.kid,
  ...
  it("Should succeed and perform API producerKeychains calls", async () => {
    const m2mProducerKeychainJWKsResponse: m2mGatewayApi.JWKs = {
      pagination: {
