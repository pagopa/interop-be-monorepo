...
describe("getVerifiedAttribute", () => {
  ...
  it("Should succeed and perform API clients calls", async () => {
    const m2mAttributeResponse: m2mGatewayApi.VerifiedAttribute = {
    ...
    const result = await attributeService.getVerifiedAttribute(
  ...
  it("Should throw attributeNotFound in case the returned attribute has an unexpected kind", async () => {
    ...
    await expect(
      attributeService.getVerifiedAttribute(
        mockAttributeProcessResponse.data.id,
        getMockM2MAdminAppContext()
      )
