...
describe("GET /producerKeychains router test", () => {
  const m2mPartialProducerKeychainsResponse: m2mGatewayApi.ProducerKeychains = {
  ...
  const m2mFullProducerKeychainsResponse: m2mGatewayApi.ProducerKeychains = {
    pagination: { offset: 0, limit: 10, totalCount: 2 },
    results: [
  ...
  const mockQueryParams: m2mGatewayApi.GetProducerKeychainsQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApi.GetProducerKeychainsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as m2mGatewayApi.GetProducerKeychainsQueryParams
    );
