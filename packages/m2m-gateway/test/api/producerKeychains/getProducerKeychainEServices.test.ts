...
describe("GET /producerKeychains/:producerKeychainId/eservices router test", () => {
  const makeRequest = async (
    ...
    query: m2mGatewayApi.GetProducerKeychainEServicesQueryParams
  ) =>
    request(api)
  ...
  const mockQueryParams: m2mGatewayApi.GetProducerKeychainEServicesQueryParams =
    {
      offset: 0,
      limit: 10,
    };
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      generateId(),
      query as m2mGatewayApi.GetProducerKeychainEServicesQueryParams
    );
