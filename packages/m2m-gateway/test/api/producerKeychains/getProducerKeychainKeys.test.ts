...
describe("GET /producerKeychains/:keychainId/keys router test", () => {
  const makeRequest = async (
    ...
    query: m2mGatewayApi.GetProducerKeychainKeysQueryParams
  ) =>
    request(api)
  ...
  const mockM2MJWKsResponse: m2mGatewayApi.JWKs = {
    pagination: { offset: 0, limit: 10, totalCount: 2 },
    results: [toM2MJWK(mockApiKey1), toM2MJWK(mockApiKey2)],
  };

  const mockQueryParams: m2mGatewayApi.GetProducerKeychainKeysQueryParams = {
    offset: 0,
    limit: 10,
  };
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      generateId(),
      query as m2mGatewayApi.GetProducerKeychainKeysQueryParams
    );
