...
describe("POST /agreements/:agreementId/submit router test", () => {
  const mockApiSubmitAgreementBody: m2mGatewayApi.AgreementSubmission = {
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      ...
      const res = await makeRequest(token, mockApiSubmitAgreementBody);
  ...
  it("Should return 400 if passed an invalid agreement submission seed", async () => {
    const token = generateToken(authRole.M2M_ADMIN_ROLE);
