...
describe("GET /agreement/:agreementId/purposes router test", () => {
  ...
  const makeRequest = async (
    token: string,
    agreementId: string,
    query: m2mGatewayApi.GetAgreementPurposesQueryParams
  ) =>
    request(api)
  ...
  const mockQueryParams: m2mGatewayApi.GetAgreementPurposesQueryParams = {
    offset: 0,
    limit: 10,
  };
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      generateId(),
      query as m2mGatewayApi.GetAgreementPurposesQueryParams
    );
