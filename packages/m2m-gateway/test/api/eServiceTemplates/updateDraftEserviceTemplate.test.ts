...
describe("PATCH /eserviceTemplates/:templateId router test", () => {
  ...
  const mockUpdateSeed: m2mGatewayApi.EServiceTemplateDraftUpdateSeed = {
    name: "updated name",
  ...
  const mockM2MEServiceTemplate: m2mGatewayApi.EServiceTemplate =
    toM2MGatewayEServiceTemplate(mockEServiceTemplate);

  const makeRequest = async (
    token: string,
    templateId: string = mockEServiceTemplate.id,
    body: m2mGatewayApi.EServiceTemplateDraftUpdateSeed = mockUpdateSeed
  ) =>
    request(api)
  ...
  ] satisfies m2mGatewayApi.EServiceTemplateDraftUpdateSeed[])(
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      mockEServiceTemplate.id,
      seed as m2mGatewayApi.EServiceTemplateDraftUpdateSeed
    );
