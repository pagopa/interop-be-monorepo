...
describe("DELETE /producerKeychains/:keychainId/eservices/:eserviceId router test", () => {
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      ...
      mockProducerKeychainService.removeEServiceFromProducerKeychain = vi.fn();
      ...
      expect(
        mockProducerKeychainService.removeEServiceFromProducerKeychain
  ...
  it.each([missingMetadata(), pollingMaxRetriesExceeded(3, 10)])(
    ...
    async (error) => {
      mockProducerKeychainService.removeEServiceFromProducerKeychain = vi
        .fn()
