...
describe("GET /producerDelegations router test", () => {
  ...
  const mockQueryParams: m2mGatewayApi.GetProducerDelegationsQueryParams = {
  ...
  const makeRequest = async (
    token: string,
    query: m2mGatewayApi.GetProducerDelegationsQueryParams
  ) =>
    request(api)
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      query as m2mGatewayApi.GetProducerDelegationsQueryParams
    );
