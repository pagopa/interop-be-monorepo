...
describe("PATCH /purposeTemplates/:purposeTemplateId router test", () => {
  ...
  const mockUpdateSeed: m2mGatewayApi.PurposeTemplateDraftUpdateSeed = {
    targetDescription: "updated target description",
  };

  const mockM2MPurposeTemplate: m2mGatewayApi.PurposeTemplate =
    toM2MGatewayApiPurposeTemplate(mockPurposeTemplate);

  const makeRequest = async (
    token: string,
    purposeTemplateId: string = mockPurposeTemplate.id,
    body: m2mGatewayApi.PurposeTemplateDraftUpdateSeed = mockUpdateSeed
  ) =>
    request(api)
  ...
  ] satisfies m2mGatewayApi.PurposeTemplateDraftUpdateSeed[])(
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      mockPurposeTemplate.id,
      seed as m2mGatewayApi.PurposeTemplateDraftUpdateSeed
    );
