...
describe("POST /eservices/:eserviceId/descriptors/:descriptorId/certifiedAttributes/groups router test", () => {
  ...
  const mockAttributeIds = [generateId(), generateId(), generateId()];
  ...
  const mockResponse: m2mGatewayApi.EServiceDescriptorCertifiedAttribute[] = [
    {
      groupIndex: 0,
  ...
  it("Should return 400 if passed invalid attribute IDs", async () => {
    ...
    const invalidBody = ["not-a-uuid", "also-invalid"];
    const res = await makeRequest(
      token,
      mockEService.id,
      mockDescriptor.id,
