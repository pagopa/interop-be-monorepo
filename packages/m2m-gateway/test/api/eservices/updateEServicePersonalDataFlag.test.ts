...
describe("POST /eservices/:eserviceId/personalDataFlag router test", () => {
  const mockEService: catalogApi.EService = getMockedApiEservice();

  const mockSeed: m2mGatewayApi.EServicePersonalDataFlagUpdateSeed = {
    personalData: randomBoolean(),
  };
  ...
  const makeRequest = async (
    ...
    body: m2mGatewayApi.EServicePersonalDataFlagUpdateSeed = mockSeed
  ) =>
    request(api)
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      mockEserviceService.updateEServicePersonalDataFlag = vi
        .fn()
      ...
      expect(
        mockEserviceService.updateEServicePersonalDataFlag
  ...
  it.each([
    ...
    { ...mockSeed, personalData: "invalid" },
  ])("Should return 400 if passed invalid seed (seed #%#)", async (seed) => {
    const token = generateToken(authRole.M2M_ADMIN_ROLE);
    const res = await makeRequest(
      token,
      mockEService.id,
      seed as m2mGatewayApi.EServicePersonalDataFlagUpdateSeed
    );
