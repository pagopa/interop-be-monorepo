...
describe("POST /eservices/{eServiceId}/descriptors/{descriptorId}/certifiedAttributes/groups/{groupIndex}/attributes router test", () => {
  ...
  const mockResponse: m2mGatewayApi.EServiceDescriptorCertifiedAttributesGroup =
    {
      attributes: [
        {
          groupIndex: 0,
  ...
  it("Should return 404 if descriptor not found", async () => {
    ...
    mockEserviceService.assignEServiceDescriptorCertifiedAttributesToGroup = vi
      .fn()
      .mockRejectedValue(
        eserviceDescriptorNotFound(
          unsafeBrandId(mockEService.id),
