...
describe("POST /eservices/:eserviceId/descriptors/:descriptorId/verifiedAttributes/groups router test", () => {
  ...
  const mockResponse: m2mGatewayApi.EServiceDescriptorVerifiedAttribute[] = [
    {
      groupIndex: 0,
  ...
  const makeRequest = async (
    ...
    request(api)
      .post(
        `${appBasePath}/eservices/${eserviceId}/descriptors/${descriptorId}/verifiedAttributes/groups`
      )
  ...
  it.each(authorizedRoles)(
    "Should return 201 and create verified attributes group for user with role %s",
    async (role) => {
      mockEserviceService.createEServiceDescriptorVerifiedAttributesGroup = vi
        .fn()
      ...
      expect(
        mockEserviceService.createEServiceDescriptorVerifiedAttributesGroup
