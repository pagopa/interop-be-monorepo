...
describe("GET /clients/:clientId/keys router test", () => {
  const makeRequest = async (
    token: string,
    clientId: string,
    query: m2mGatewayApi.GetClientKeysQueryParams
  ) =>
    request(api)
  ...
  const mockQueryParams: m2mGatewayApi.GetClientKeysQueryParams = {
    offset: 0,
    limit: 10,
  };
  ...
  it.each([
    ...
    const res = await makeRequest(
      token,
      generateId(),
      query as m2mGatewayApi.GetClientKeysQueryParams
    );
