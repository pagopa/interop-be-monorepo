...
describe("GET /clients/:clientId/purposes router test", () => {
  const makeRequest = async (
    token: string,
    clientId: string,
    query: m2mGatewayApi.GetClientPurposesQueryParams
  ) =>
    request(api)
  ...
  const mockQueryParams: m2mGatewayApi.GetClientPurposesQueryParams = {
    offset: 0,
    limit: 10,
    eservicesIds: [],
    states: [],
  ...
  it.each(authorizedRoles)(
    ...
    async (role) => {
      ...
      mockClientService.getClientPurposes = vi
        .fn()
      ...
      expect(mockClientService.getClientPurposes).toHaveBeenCalledWith(
        clientId,
        mockQueryParams,
  ...
  it.each([
    ...
    { ...mockQueryParams, eservicesIds: ["INVALID_ID"] },
  ...
  it.each([
    ...
    async (resp) => {
      mockClientService.getClientPurposes = vi.fn().mockResolvedValueOnce(resp);
      const token = generateToken(authRole.M2M_ADMIN_ROLE);
      const res = await makeRequest(token, generateId(), mockQueryParams);
