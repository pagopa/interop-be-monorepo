...
export function purposeServiceBuilder(clients: PagoPAInteropBeClients) {
  ...
  return {
    async getPurposes(
      ...
      const { eserviceIds, title, consumersIds, states, limit, offset } =
        queryParams;

      logger.info(
        `Retrieving purposes for eServiceIds ${eserviceIds}, title ${title}, consumersIds ${consumersIds}, states ${states}, limit ${limit} offset ${offset}`
      );
