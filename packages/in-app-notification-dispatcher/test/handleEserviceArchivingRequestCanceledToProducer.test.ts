...
import { handleEserviceArchivingRequestCanceledToProducer } from "../src/handlers/eservices/handleEserviceArchivingRequestCanceledToProducer.js";
import {
  addOneDelegation,
  addOneEService,
  addOneTenant,
...
describe("handleEserviceArchivingRequestCanceledToProducer", () => {
  ...
  describe("EServiceArchivingRequestCanceledByDelegateV2", async () => {
    ...
    it("returns empty array when no users have notifications enabled", async () => {
      ...
      const notifications =
        await handleEserviceArchivingRequestCanceledToProducer(
          msg,
          logger,
          readModelService
        );
    ...
    it("emits a notification for EServiceArchivingRequestCanceledByDelegate", async () => {
      ...
      expect(notifications[0]).toEqual({
        ...
        body: inAppTemplates.eserviceArchivingRequestCanceledToProducer(
  ...
  describe("EServiceDescriptorArchivingRequestCanceledByDelegate", async () => {
    it("emits a notification for EServiceDescriptorArchivingRequestCanceledByDelegate", async () => {
      ...
      expect(notifications[0]).toEqual({
        ...
        notificationType: "eserviceArchivingRequestedToDelegator",
        entityId: `${eservice.id}/${descriptorId}`,
        body: inAppTemplates.eserviceDescriptorArchivingRequestCanceledToProducer(
          delegatorTenant.name,
          eservice.name,
          descriptor.version
        ),
