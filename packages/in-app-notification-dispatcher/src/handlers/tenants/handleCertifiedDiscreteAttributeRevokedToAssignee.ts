...
export async function handleCertifiedDiscreteAttributeRevokedToAssignee(
  ...
  logger.info(
    `Sending in-app notification for handleCertifiedDiscreteAttributeRevokedToAssignee - entityId: ${tenantV2Msg.id}, eventType: TenantCertifiedDiscreteAttributeRevoked`
  );
  ...
  if (usersWithNotifications.length === 0) {
    logger.info(
      `No users with notifications enabled for handleCertifiedDiscreteAttributeRevokedToAssignee - entityId: ${tenant.id}, eventType: TenantCertifiedDiscreteAttributeRevoked`
    );
    return [];
  }
