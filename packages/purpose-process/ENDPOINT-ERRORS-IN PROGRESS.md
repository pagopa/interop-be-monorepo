...
| `riskAnalysisValidationFailed` | 400 | `validateRiskAnalysisOrThrow` rejects the risk-analysis form when the submitted answers fail schema or business validation. | **CAN HAPPEN** — the user can submit an incomplete or invalid risk-analysis form from the normal purpose-creation journey. | **Data precondition:** you are consumer A with an active agreement for e-service E, and the risk-analysis section is enabled for that purpose.<br>1. Go to the consumer purpose creation page from the side menu.<br>2. Select tenant A and e-service E.<br>3. Fill the purpose title/description and complete the risk-analysis section with a missing or invalid answer.<br>4. Click _Crea bozza_ or the final save action. | 🟡 Medium resolution <br />1. Correct the invalid or missing risk-analysis answers before submitting.<br>2. Re-open the risk-analysis step and re-validate the required fields.<br>3. Save only after the form shows a valid state. |
...
| `riskAnalysisValidationFailed` | 400 | `validateRiskAnalysisOrThrow` rejects the risk-analysis answers for the reverse-purpose creation flow. | **CAN HAPPEN** — the user can submit a receive-mode purpose with an incomplete or invalid risk-analysis payload through the normal UI flow. | **Data precondition:** consumer A has an active agreement for receive-mode e-service E and a valid starter risk-analysis template.<br>1. Go to the receive-purpose creation page from the side menu.<br>2. Select tenant A and e-service E.<br>3. Leave one required risk-analysis answer empty or set an invalid value.<br>4. Click the final save action. | 🟡 Medium resolution <br />1. Complete the missing or invalid risk-analysis answers.<br>2. Re-run validation on the analysis form.<br>3. Save only after the form is valid. |
...
| `riskAnalysisValidationFailed` | 400 | `validateRiskAnalysisOrThrow` rejects an incomplete or invalid risk-analysis form. | **CAN HAPPEN** — the purpose edit flow allows the user to submit a risk-analysis with a missing or invalid answer through the normal wizard. | **Data precondition:** you are tenant A and the draft purpose already exists; its e-service requires a risk-analysis and the form is editable.<br>1. Go to `/fruizione/finalita/:purposeId/modifica`.<br>2. Open the step _Analisi del rischio_.<br>3. Leave a required answer empty or enter an invalid value.<br>4. Click _Salva bozza e prosegui_. | 🟡 Medium resolution <br />1. Complete or correct the invalid risk-analysis answers.<br>2. Re-run the validation on the form.<br>3. Save again only after the form is valid. |
...
| `riskAnalysisValidationFailed` | 400 | `validateAndTransformRiskAnalysis(...)` rejects the submitted risk-analysis payload when schema or business validation fails. | **CAN HAPPEN** — the normal purpose-edit flow lets a user submit an incomplete or invalid risk-analysis answer from the _Analisi del rischio_ step. | **Data precondition:** consumer A has a draft purpose for e-service E in the `adminWritesReviewerSigns` flow and the risk-analysis step is editable.<br>1. Go to `/fruizione/finalita/:purposeId/modifica`.<br>2. Open the _Analisi del rischio_ step.<br>3. Leave one required answer empty or enter a value that fails validation (for example, a personal-data answer that conflicts with the e-service declaration).<br>4. Click _Richiedi approvazione_. | 🟡 Medium resolution <br />1. Complete or correct the invalid risk-analysis answers.<br>2. Re-run validation on the form.<br>3. Submit again only after the form is valid. |
...
| `riskAnalysisValidationFailed` | 400 | `validateAndTransformRiskAnalysis` rejects the submitted seed when the risk-analysis answers are structurally or semantically invalid. | **CAN HAPPEN** — the edit form lets the reviewer submit a partially filled or invalid answer set through the normal review-flow UI. | **Data precondition:** the review workflow for purpose A is assigned and still editable; the reviewer is the designated reviewer for that purpose.<br>1. Go to the purpose summary and open the risk-analysis compilation step for purpose A.<br>2. Leave one required answer empty or enter an invalid value in a field such as a required free-text response or a wrong multiple-choice value.<br>3. Click _Salva bozza e prosegui_. | 🟡 Medium resolution <br />1. Complete or correct the invalid risk-analysis answers.<br>2. Re-run validation on the form.<br>3. Save again only after the form is valid. |
...
| Error | Status | When it happens | Reachable from the FE? | Steps to reproduce (UI) | Resolution steps |
| ----- | ------ | --------------- | -------------------- | ---------------------- | ---------------- |
| `missingRiskAnalysis` | 400 | The draft purpose version is activated without a risk-analysis form attached to the current purpose record. | **CAN HAPPEN (UNVERIFIED)** — the activation action is offered from the purpose summary, but the UI still needs to be checked to confirm whether the draft can be activated without an attached risk-analysis form in the normal flow. | **Data precondition:** you have a draft purpose A for e-service E and no risk-analysis form is attached.<br>1. Open the purpose details page for A.<br>2. Click _Attiva_.<br>3. Confirm the activation dialog. | 🟡 Medium resolution <br />1. Open the risk-analysis step and complete the required answers.<br>2. Save the purpose draft again.<br>3. Retry the activation action after the form is present. |
...
| `riskAnalysisValidationFailed` | 400 | `validateRiskAnalysisOrThrow` rejects the current risk-analysis form when it is invalid or stale at activation time. | **CAN HAPPEN (UNVERIFIED)** — the activation flow may still reach the backend with a stale or invalid risk-analysis payload during a manual draft update, and the UI must be checked to confirm the button stays enabled in that state. | **Data precondition:** you have a purpose draft A whose risk-analysis form is stale, incomplete, or no longer valid for the selected e-service.<br>1. Open the purpose summary page for A.<br>2. Click _Attiva_.<br>3. Confirm the activation dialog. | 🟡 Medium resolution <br />1. Refresh the purpose details page and reopen the risk-analysis step.<br>2. Correct the invalid answers and re-save the risk analysis.<br>3. Retry activation once the validation passes. |
...
| `riskAnalysisValidationFailed` | 400 | `validateRiskAnalysisAgainstTemplateOrThrow` rejects an incomplete or invalid risk-analysis answer set inherited from the purpose template. | **CAN HAPPEN** — the user can submit a template-created purpose with an invalid or incomplete risk-analysis form through the standard creation flow. | **Data precondition:** tenant A is the consumer for e-service E and the template selected for creation includes a risk-analysis form that must be completed.<br>1. Go to `/fruizione/finalita/:purposeTemplateId/crea`.<br>2. Select e-service E and confirm the template-based flow.<br>3. Leave a required answer empty or enter a value that does not match the template constraints.<br>4. Click _Crea bozza_. | 🟡 Medium resolution <br />1. Complete the missing or invalid risk-analysis answers.<br>2. Re-run validation on the form.<br>3. Submit the draft again after the form is valid. |
...
| Error | Status | When it happens | Reachable from the FE? | Steps to reproduce (UI) | Resolution steps |
| ----- | ------ | --------------- | -------------------- | ---------------------- | ---------------- |
| `riskAnalysisValidationFailed` | 400 | `validateRiskAnalysisAgainstTemplateOrThrow` rejects the submitted risk-analysis form when it is incomplete or invalid. | **CAN HAPPEN** — the edit form allows the user to save a risk-analysis draft with a missing or invalid answer through the normal template-based update flow. | **Data precondition:** you are tenant A, the template-based draft already exists, and the risk-analysis form is still editable.<br>1. Go to `/fruizione/template-finalita/:purposeTemplateId/modifica` or the matching purpose template edit route.<br>2. Open the general-information step and leave a required answer empty or enter an invalid value.<br>3. Click _Salva bozza e prosegui_. | 🟡 Medium resolution <br />1. Complete or correct the invalid risk-analysis answers.<br>2. Re-run validation on the form.<br>3. Save again only after validation succeeds. |
