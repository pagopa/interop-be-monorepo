...
export function authorizationPlatfromStateReadModelServiceBuilder(
  tokenGenerationReadModelRepository: TokenGenerationReadModelRepository
) {
  const { dynamoDBClient } = tokenGenerationReadModelRepository;
  const commonTokenGenerationReadModelService =
    tokenGenerationCommonReadModelServiceBuilder(
      tokenGenerationReadModelRepository
    );
  return {
    async deleteEntriesFromTokenStatesByKid(
      GSIPK_kid: GSIPKKid
      ...
      const runPaginatedQuery = async (
        ...
        if (!data.Items) {
          throw genericInternalError(
            `Unable to read token state entries: result ${JSON.stringify(
              data
            )} `
          );
        } else {
          ...
          for (const entry of tokenStateEntries.data) {
            await this.deleteClientEntryFromTokenGenerationStatesTable(entry);
          }
    ...
    async deleteClientEntryFromPlatformStates(
    ...
    async deleteEntriesFromTokenStatesByClient(
    ...
    async deleteClientEntryFromTokenGenerationStatesTable(
    ...
    async deleteEntriesWithClientAndPurposeFromTokenGenerationStatesTable(
    ...
    async convertEntriesToClientKidInTokenGenerationStates(
      GSIPK_clientId_purposeId: GSIPKClientIdPurposeId
    ): Promise<void> {
      const runPaginatedQuery = async (
        GSIPK_clientId_purposeId: GSIPKClientIdPurposeId,
        ...
        for (const entry of res.tokenStateEntries) {
          ...
          // write the new one
          await commonTokenGenerationReadModelService.writeTokenStateClientEntry(
            newEntry
          );
    ...
    async readPlatformAgreementEntryByGSIPKConsumerIdEServiceId(
    ...
    async readPlatformPurposeEntry(
    ...
    async upsertTokenStateClientPurposeEntry(
    ...
    async readClientEntriesInTokenGenerationStates(
    ...
    async setClientPurposeIdsInPlatformStatesEntry(
      primaryKey: PlatformStatesClientPK,
      version: number,
    ...
    async retrievePlatformStatesByPurpose(purposeId: PurposeId): Promise<{
      ...
      const purposeEntry = await this.readPlatformPurposeEntry(purposePK);
      ...
      const agreementEntry =
        await this.readPlatformAgreementEntryByGSIPKConsumerIdEServiceId(
          agreementGSI
        );
      ...
      const catalogEntry = await this.readCatalogEntry(catalogPK);
    ...
    async upsertPlatformClientEntry(
    ...
    async upsertTokenClientKidEntry(
...
export type AuthorizationPlatfromStateReadModelService = ReturnType<
  typeof authorizationPlatfromStateReadModelServiceBuilder
>;
