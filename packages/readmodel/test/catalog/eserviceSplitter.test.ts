...
describe("E-service splitter", () => {
  it.each([...gracePeriodDays])(
    "should convert a complete e-service into e-service SQL objects (gracePeriodDays: %d)",
    (gracePeriodDaysValue: GracePeriodDays) => {
      ...
      const expectedTemplateVersionRef: EServiceDescriptorTemplateVersionRefSQL =
        {
          ...
          contactName:
            templateVersionRef.interfaceMetadata?.contactName ?? null,
          contactEmail:
      ...
      expect(templateVersionRefsSQL).toStrictEqual([
        expectedTemplateVersionRef,
      ]);
